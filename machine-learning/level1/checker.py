...
def step_2_check(model):
    global _correct_count
    try:
        assert type(model) == type(LinearRegression()), f"Expected `model` to be of type LinearRegressor but got an object of type `{type(model)}`"
...
def step_4_check(mse):
    global _correct_count
    try:
        assert mse == _Solution.mse, "`mse` doesn't match the expected value, did you pass the correct values to `mean_squared_error()`?"
    except AssertionError as e:
        print(_color_text('Incorrect:', 'red'), e)
